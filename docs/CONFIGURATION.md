...
- Certificate lifecycle: `api ssl cert info` reports both H3 and ACME
  certificates; add `--target h3` or `--target acme` to inspect one.
  `generate|renew` require `--target h3` or `--target acme`;
  `--acme-key-authorization` is accepted only for `--target acme`. ACME
  generation requires a DNS hostname from `--domain` or `ssl.domain`; it does
  not fall back to `localhost`. On startup, mlai-trade auto-renews only
  mlai-trade-generated certificates when `ssl.cert_mode=self_signed`;
  provided/public CA certificates are never overwritten. The ACME TLS-ALPN-01 challenge
