...
- `ssl.auth.enabled`: require HTTP Basic auth for non-localhost remote clients.
...
- `ssl.auth.username` / `ssl.auth.password`: remote dashboard/API credentials.
  Localhost source traffic bypasses auth so `https://localhost:5443/` works as
  a local dashboard without a login prompt.
...
- `ssl.tcp_bootstrap_enabled`: enables the browser discovery bootstrap
  listener, default `true`. It serves only a TLS 1.3 HTTP/1.1 response with
  `Alt-Svc: h3=":5443"` and exposes no API routes.
- `ssl.tcp_bootstrap_bind_host` and `ssl.tcp_bootstrap_port`: bootstrap
  listener address, defaulting to the QUIC bind host and UDP port.
...
- Certificate lifecycle: `api ssl cert info --target all|h3|acme` reports
...
`5443` when possible. The TCP bootstrap listener exists for browsers that first
try TCP/TLS and need to learn `Alt-Svc` before switching to H3. Normal TCP API
or dashboard payload serving is intentionally not provided; apps without H3
support fail closed.
...
`api/html/dist`, which the H3 listener serves. `api/html/robots.txt` disallows
all crawlers and common AI-agent user agents, but that is advisory only; auth is
the real protection for non-localhost clients.
